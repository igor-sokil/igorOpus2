...
#define UN_PHS   57000
static char chStpfault_U = 0, po_I2_DZ__I2_LESS_IN = 0; 
//=====================================================================================================
//''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
//                  
//....................................................................................................
...
register union { 
  ...
  long lVl;
   ...
   if( (measurement[IM_IA] < (unsigned long) i)
    ...
    if(chStpfault_U&2)
        i = (U_F_NOM*2*U_DOWN)/1000;
    else
        i = (U_F_NOM*2)/10;    
    
  if( (measurement[IM_UA] < (unsigned long)i)
       &&  (measurement[IM_UB] < (unsigned long)i)
       &&  (measurement[IM_UC] < (unsigned long)i)
      ){
  ...
  if( (u32_bit_holder & (1<<FAULT_U_0DOT5_T_STATE_BIT)) != 0 )
   ...
   if (i > (j)){
        //?wrp.bool_vars.AND3_2 = 1;<-old code
        wrp.bool_vars.AND2_3 = 1;chStpfault_U |= 8;
    }
    else
        chStpfault_U &= 0xf7;
      
    if( po_I2_DZ__I2_LESS_IN == 0)
      i = I_NOM;
    else 
      i =  (I_NOM*11/10);//5500
     if(measurement[IM_I2] < (unsigned)i){
       wrp.bool_vars.AND3_2 = 1;
       po_I2_DZ__I2_LESS_IN = 1;
     }
     else{
       po_I2_DZ__I2_LESS_IN = 0;
     }
...
char chGbl_3Phs_Dz; char fix_state_SNKN_DZ_1, state_SNKN_DZ_1_was;
//=====================================================================================================
//''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
//                  
//....................................................................................................
...
struct{
    char  ch_fault_branch_dz1;
}sLV;
    wrp_sncn.lVl = 0;sLV.ch_fault_branch_dz1 = 0;
...
//    wrp_sncn.bool_vars.not7_1 = !wrp_sncn.bool_vars.and5_1;//Inv 3phs
    ...
    if( _CHECK_SET_BIT(p_active_functions, RANG_NKN_DZ)
        )//!-&& (wrp_sncn.bool_vars.not7_1) blockshema change
...
//Calc  for 1 stages 7-9 elem
  ...
  u32_bit_holder, AMTZ1_PICKUP_STATE_BIT, u32_bit_holder, AMTZ1_TMR_STATE_BIT);
    ...
    if(lV&(1<<0)){//One
        rU = 0;
        if(
            (fix_state_SNKN_DZ_1 != 0)//Було спрацювання пуск органу
            && state_SNKN_DZ_1_was != 0 //була аварійна гілка
        ...
        if(   (
                wrp_sncn.bool_vars.or6_O1&&wrp_dz1.bool_vars.not3_1
              )
              && (fix_state_SNKN_DZ_1 == 0)
        ){
          rU ++;
        }
        if(
        ...
        //if(wrp_sncn.bool_vars.or6_O1&&wrp_dz1.bool_vars.not3_1)
        {//Check Fault and2
            rU = Za_A;
            if(lV&(1<<5))
                rU = Za_B;
...
//nor6  
  ...
  u32_bit_holder, DZ1_PICKUP_STATE_BIT, u32_bit_holder, DZ1_TMR_STATE_BIT);
        if( wrp_dz1.bool_vars.not10_1 == 0 )//Fix set 0->1
        {
          fix_state_SNKN_DZ_1 |= 1;
          if( sLV.ch_fault_branch_dz1!= 0)
            state_SNKN_DZ_1_was |= 1;
        }  
...
char fix_state_SNKN_DZ_2, state_SNKN_DZ_2_was;
char fix_state_SNKN_DZ_3, state_SNKN_DZ_3_was;
char fix_state_SNKN_DZ_4, state_SNKN_DZ_4_was;
/*
...
// ----------------  inner_outer_short_circuit It`s my name -------------------------
struct{
    char  ch_stage_selector, ch_fault_branch_dz2,ch_fault_branch_dz3,ch_fault_branch_dz4;
...
sLV.p2.lVl = 0;                                      
 sLV.ch_fault_branch_dz2 = 0;                                 
 sLV.ch_fault_branch_dz3 = 0;                                 
 sLV.ch_fault_branch_dz4 = 0;                                    
...
//          }
            ...
            {//One
                ...
                if(
                    (fix_state_SNKN_DZ_2 != 0)//Було спрацювання пуск органу
                    && state_SNKN_DZ_2_was != 0 //була аварійна гілка
                      
                )
                  rU ++;
                
                if(   (
                        (sLV.p_p4->lVl&((1<<20)|(1<<21))) == ((1<<20)|(1<<21))
                      )
                      && (fix_state_SNKN_DZ_2 == 0)
                )
                  rU ++;
                if(
                     rU != 0 //(sLV.p_p4->lVl&((1<<20)|(1<<21))) == ((1<<20)|(1<<21))
...
p2.bool_vars.and24_i_2 = 1;  
            ...
            if(rU == ((1<<4)|(1<<5)|(1<<6)|(1<<7)|(1<<8))){//and24 4 5 6 7 8
                ...
                if(p2.bool_vars.not25_o_0 == 1)//Fix set 0->1
                {
                  
                  fix_state_SNKN_DZ_2 |= 2;
                  if( sLV.ch_fault_branch_dz2!= 0)
                    state_SNKN_DZ_2_was |= 2;
                }
            ...
            if(rU == ((1<<0)|(1<<30)|(1<<1)|(1<<2)|(1<<3))){//and22 0 1 2 3
                ...
                if( p2.bool_vars.not23_o_0 == 1 )//Fix set 0->1
                {
                  fix_state_SNKN_DZ_2 |= 1;
                  if( sLV.ch_fault_branch_dz2!= 0)
                    state_SNKN_DZ_2_was |= 1;
                }  
            }
            else
            {
            ...
            rU = sLV.ch_fault_branch_dz2 ; //_CHECK_SET_BIT(p_active_functions, RANG_NKN_DZ) != 1;
            ...
            //?  )
            //?  state_SNKN_DZ_2_was  |= 2; 
            //?if(fix_state_SNKN_DZ_2 == 0)
            //?  state_SNKN_DZ_2_was  = 0;
...
//            }
            ...
            {//One
                ...
                if(
                    (fix_state_SNKN_DZ_3 != 0)//Було спрацювання пуск органу
                    && state_SNKN_DZ_3_was != 0 //була аварійна гілка
                ...
                //.?if((sLV. p_p4->lVl&((1<<20)|(1<<21))) == ((1<<20)|(1<<21)))
                {//Check Fault and11   sLV.p_p4->bool_vars.and11_o_0
                    rU = Za_A;
                    if(lV&(1<<5))
                        rU = Za_B;
                    else if(lV&(1<<6))
            ...
            if(rU == ((1<<4)|(1<<5)|(1<<6)|(1<<7)|(1<<8))){//and24 4 5 6 7 8
                ...
                if(p2.bool_vars.not25_o_0 == 1)//Fix set 0->1
                {
                  ...
                  fix_state_SNKN_DZ_3 |= 2;
                  if( sLV.ch_fault_branch_dz3!= 0)
                    state_SNKN_DZ_3_was |= 2;
                }
            ...
            if(rU == ((1<<0)|(1<<30)|(1<<1)|(1<<2)|(1<<3))){//and22 0 1 2 3
                ...
                if( p2.bool_vars.not23_o_0 == 1 )//Fix set 0->1
                {
                  fix_state_SNKN_DZ_3 |= 1;
                  if( sLV.ch_fault_branch_dz3!= 0)
                    state_SNKN_DZ_3_was |= 1;
                }  
...
//            }
            ...
            lV = m_s_phs_stp_state.lVl;rU = 0;
            if(lV&(1<<0))
            {//One
              if(
                    (fix_state_SNKN_DZ_4 != 0)//Було спрацювання пуск органу
                    && state_SNKN_DZ_4_was != 0 //була аварійна гілка
...
p2.bool_vars.and22_i_2 = 1;  //? p2.bool_vars.and24_i_2 = 1; 
            ...
            if(rU == ((1<<4)|(1<<5)|(1<<6)|(1<<7)|(1<<8))){//and24 4 5 6 7 8
                ...
                if(p2.bool_vars.not25_o_0 == 1)//Fix set 0->1
                {
                  ...
                  fix_state_SNKN_DZ_4 |= 2;
                  if( sLV.ch_fault_branch_dz4!= 0)
                    state_SNKN_DZ_4_was |= 2;
                }
            ...
            if(rU == ((1<<0)|(1<<30)|(1<<1)|(1<<2)|(1<<3))){//and22 0 1 2 3
                ...
                if( p2.bool_vars.not23_o_0 == 1 )//Fix set 0->1
                {
                  fix_state_SNKN_DZ_4 |= 1;
                  if( sLV.ch_fault_branch_dz4!= 0)
                    state_SNKN_DZ_4_was |= 1;
                }  
