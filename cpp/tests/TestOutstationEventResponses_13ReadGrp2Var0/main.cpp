...
void configure_in_13ReadGrp2Var0(DatabaseConfig*);
void configure_in_13ReadGrp2Var0(DatabaseConfig* db)
{
}

void update_in_13ReadGrp2Var0(IUpdateHandler*);
void update_in_13ReadGrp2Var0(IUpdateHandler* db)
{
    std::cout << "update_in_13ReadGrp2Var0" <<'\n';
...
//    DatabaseConfig tmp = binary_input_in_DatabaseHelpers(1);
  ...
  configure_in_13ReadGrp2Var0(&database);
...
////    t.Transaction([&](IUpdateHandler& db) { loadFun(db); });
    Transaction_in_OutstationTestObject(&t, update_in_13ReadGrp2Var0);//void (*apply)(IUpdateHandler*));

////    t.SendToOutstation(request);
    uint8_t name1[] = {5, 0xC0, 0x01, 0x02, 0x00, 0x06};
    SendToOutstation_in_OutstationTestObject(&t, name1);  
////    REQUIRE(t.lower->PopWriteAsHex() == response);
qDebug()<<"REQUIRE('E0 81 80 00 02 01 28 01 00 00 00 01' == t.lower->PopWriteAsHex())";

/*
TEST_CASE(SUITE("13ReadGrp2Var0"))
{
    auto update = [](IUpdateHandler& db) { db.Update(Binary(false, Flags(0x01)), 0); };
