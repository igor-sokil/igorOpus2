...
////    auto configure = [](DatabaseConfig& db) { db.counter[0].evariation = EventCounterVariation::Group22Var5; };
        db->counter_config[0].dDeadbandConfig_for_CounterInfo.eEventConfig.evariation = EventCounterVariation_Group22Var5;
}
...
//    DatabaseConfig tmp = binary_input_in_DatabaseHelpers(1);
  ...
  configure_in_11reports(&database);
...
////    t.Transaction([&](IUpdateHandler& db) { loadFun(db); });
    Transaction_in_OutstationTestObject(&t, update_in_11reports);//void (*apply)(IUpdateHandler*));
...
qDebug()<<"REQUIRE('E0 81 80 00 16 05 28 01 00 00 00 01 17 00 00 00 78 E6 B7 2D 60 01' == t.lower->PopWriteAsHex())";
