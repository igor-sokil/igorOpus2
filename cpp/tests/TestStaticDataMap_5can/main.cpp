...
qDebug()<<"********SUITE('5can ignore events on existing point')********";
...
std::cout << "REQUIRE(map.update(Binary(true), 0, EventMode::Suppress, receiver))"<<'\n';
...
std::cout << "temp= " << (map.map[0].selection_in_StaticDataCell_for_Binary.value_in_SelectedValue_for_BinarySpec.tTypedMeasurement_for_Boolean.value == true)<<'\n';
