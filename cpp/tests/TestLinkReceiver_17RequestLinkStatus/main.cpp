...
qDebug()<<"********SUITE('17RequestLinkStatus')********";
////    Buffer buffer(292);
...
////    auto frame = LinkFrame::FormatRequestLinkStatus(writeTo, true, 1, 2, nullptr);
RSeq_for_Uint16_t frame = FormatRequestLinkStatus_in_LinkFrame_static(
  &writeTo, true, 1, 2);//, Logger* pLogger);
...
boolean tmp = CheckLast_in_MockFrameSink(&(t.sink),
    LinkFunction_PRI_REQUEST_LINK_STATUS, true, 1, 2);
qDebug()<<"REQUIRE(t.sink.CheckLast(LinkFunction::PRI_REQUEST_LINK_STATUS, true, 1, 2))";
qDebug()<<"tmp= "<<tmp;
