...
qDebug()<<"********SUITE('16ResetLinkStates')********";
////    Buffer buffer(292);
...
////    auto frame = LinkFrame::FormatResetLinkStates(writeTo, false, 1, 2, nullptr);
RSeq_for_Uint16_t frame = FormatResetLinkStates_in_LinkFrame_static(
  &writeTo, false, 1, 2);//, Logger* pLogger);
...
boolean tmp = CheckLast_in_MockFrameSink(&(t.sink),
    LinkFunction_PRI_RESET_LINK_STATES, false, 1, 2);
qDebug()<<"REQUIRE(t.sink.CheckLast(LinkFunction::PRI_RESET_LINK_STATES, false 1, 2))";
qDebug()<<"tmp= "<<tmp;
