...
void update_in_27same_header(IUpdateHandler* db);
void update_in_27same_header(IUpdateHandler* db)
{
    OctetString data;
...
//btmp = Update_OctetString_in_IUpdateHandler(itmp, &data, 2, EventMode_Detect);// = EventMode::Detect) = 0;
 Update_for_OctetString_in_Database_override(db, &data, 2, EventMode_Detect);

//IUpdateHandler* itmp = GetUpdateHandler_in_OContext(&(t.context));
 Update_for_OctetString_in_Database_override(db, &data, 1, EventMode_Suppress);// = EventMode::Detect) = 0;
}
...
DatabaseConfig tmp = octet_string_in_DatabaseHelpers(5);
  ...
  setDataMapKeys_for_OctetStringSpec(&tmp, 2, 2);
    ...
    Transaction_in_OutstationTestObject(&t, update_in_27same_header);//void (*apply)(IUpdateHandler*));
...
qDebug()<<"REQUIRE(t.lower->PopWriteAsHex() == 'E0 81 80 00 6F 02 28 02 00 02 00 CA FE 01 00 CA FE')";
//std::cout << "temp= " << temp<<'\n';
