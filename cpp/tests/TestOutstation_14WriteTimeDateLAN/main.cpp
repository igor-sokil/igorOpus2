...
qDebug()<<"********SUITE('14WriteTimeDateLAN')********";
...
//    std::string name("C1 02 32 03 07 01 D2 04 00 00 00 00");       
    uint8_t name[] = {8, 0xC1, 0x02, 0x32, 0x03, 0x07, 0x01, 0xD2, 0x04, 0x00, 0x00, 0x00, 0x00};
    SendToOutstation_in_OutstationTestObject(&t, name);  

//    std::string temp = PopWriteAsHex_in_MockLowerLayer(&(t.lower));

qDebug()<<"REQUIRE(t.lower->PopWriteAsHex() == 'C1 81 80 04')";
//std::cout << "temp= " << temp<<'\n';

/*
TEST_CASE(SUITE("14WriteTimeDateLAN - Rejects write without prior record current time"))
//Отклоняет запись без предварительной записи текущего времени.
{
    OutstationConfig config;
