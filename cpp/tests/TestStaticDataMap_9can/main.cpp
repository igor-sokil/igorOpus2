...
qDebug()<<"********SUITE('9can select a range and iterate')********";
...
Range rng = From_in_Range_static(0, 9);
uint16_t ctmp = select_in_StaticDataMap_for_BinarySpecOver1(&map, rng);
std::cout << "REQUIRE(map.select(Range::From(0, 9)) == 3)"<<'\n';
std::cout << "ctmp= " << ctmp<<'\n';

////    const auto selected = map.get_selected_range();
Range selected = get_selected_range_in_StaticDataMap_for_BinarySpec(&map);

////    REQUIRE(selected.start == 1);
std::cout << "REQUIRE(selected.start == 1)"<<'\n';
std::cout << "selected.start= " << selected.start<<'\n';

////    REQUIRE(selected.stop == 7);
std::cout << "REQUIRE(selected.stop == 7)"<<'\n';
