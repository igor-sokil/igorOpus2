...
////    };
        db->binary_input_config[0].eEventConfig.clazz = PointClass_Class1;
        db->binary_input_config[1].eEventConfig.clazz = PointClass_Class2;
        db->binary_input_config[2].eEventConfig.clazz = PointClass_Class3;
}
...
//    DatabaseConfig tmp = binary_input_in_DatabaseHelpers(1);
  ...
  configure_in_10MixedClassLimitedCount(&database);
...
////    t.Transaction([&](IUpdateHandler& db) { loadFun(db); });
    Transaction_in_OutstationTestObject(&t, update_in_10MixedClassLimitedCount);//void (*apply)(IUpdateHandler*));

////    t.SendToOutstation(request);
    uint8_t name1[] = {10, 0xC0, 0x01, 0x3C, 0x03, 0x07, 0x01, 0x3C, 0x04, 0x07, 0x01};
    SendToOutstation_in_OutstationTestObject(&t, name1);  
////    REQUIRE(t.lower->PopWriteAsHex() == response);
qDebug()<<"REQUIRE('E0 81 8E 00 02 01 28 02 00 01 00 81 02 00 81' == t.lower->PopWriteAsHex())";
