...
void configure_in_17ReadGrp32Var5_1(DatabaseConfig*);
void configure_in_17ReadGrp32Var5_1(DatabaseConfig* db)
{
    std::cout << "configure_in_17ReadGrp32Var5_1_1" <<'\n';
...
void configure_in_17ReadGrp32Var5_2(DatabaseConfig*);
void configure_in_17ReadGrp32Var5_2(DatabaseConfig* db)
{
////                  [](DatabaseConfig& db) { db.analog_input[0].evariation = EventAnalogVariation::Group32Var5; });
  db->analog_input[0].dDeadbandConfig_for_AnalogInfo.eEventConfig.evariation = EventAnalogVariation_Group32Var5;
}

void update_in_17ReadGrp32Var5(IUpdateHandler*);
void update_in_17ReadGrp32Var5(IUpdateHandler* db)
{
    std::cout << "update_in_17ReadGrp32Var5_1" <<'\n';
////    auto update = [](IUpdateHandler& db) { db.Update(Analog(0.0, Flags(0x01), DNPTime(0x010203040506)), 0); };
 ...
 Update_Analog_in_IUpdateHandler(db, &aAnalog, 0, EventMode_Detect);// = EventMode::Detect) = 0;
    std::cout << "update_in_17ReadGrp32Var5_2" <<'\n';
}
...
////                  [](DatabaseConfig& db) { db.analog_input[0].evariation = EventAnalogVariation::Group32Var1; });
 ...
 std::string response("E0 81 80 00 20 05 28 01 00 00 00 01 00 00 00 00");       

 std::string request("C0 01 20 05 06");       
 TestEventRead(request, response,
                   ////const std::function<void(IUpdateHandler& db)>& loadFun,
                   update_in_17ReadGrp32Var5,
                   ////const std::function<void(DatabaseConfig& db)>& configure = [](DatabaseConfig& view) {})
                   configure_in_17ReadGrp32Var5_1);
...
////                  [](DatabaseConfig& db) { db.analog_input[0].evariation = EventAnalogVariation::Group32Var5; });
 ...
 TestEventRead(request_2, response,
                   ...
                   update_in_17ReadGrp32Var5,
                   ////const std::function<void(DatabaseConfig& db)>& configure = [](DatabaseConfig& view) {})
                   configure_in_17ReadGrp32Var5_2);
