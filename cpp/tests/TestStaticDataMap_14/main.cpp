...
qDebug()<<"********SUITE('14selecting the lower bound only returns a partial range')********";
...
//std::cout << "map.size()= " << map.map.size()<<'\n';
    ...
    DatabaseConfig config = binary_input_in_DatabaseHelpers(2);

   StaticDataMap_for_BinarySpec_in_StaticDataMap_for_BinarySpecOver2(&map, &config);

    getDataMapKeys_for_BinarySpec()[0] = 1;
    getDataMapKeys_for_BinarySpec()[1] = 7;
...
Range rng = From_in_Range_static(1, 6);

  StaticDataCell_for_Binary stmp = map.map[0];
  std::cout << "stmp.selection.selected= " << stmp.selection_in_StaticDataCell_for_Binary.selected_in_SelectedValue_for_BinarySpec<<'\n';
   stmp = map.map[1];
  std::cout << "stmp.selection.selected= " << stmp.selection_in_StaticDataCell_for_Binary.selected_in_SelectedValue_for_BinarySpec<<'\n';

uint16_t ctmp = select_in_StaticDataMap_for_BinarySpecOver1(&map, rng);
std::cout << "REQUIRE(map.select(Range::From(2, 6)) == 0)"<<'\n';
std::cout << "ctmp= " << ctmp<<'\n';
...
выбор диапазона, который ничему не соответствует, возвращает Range::Invalid
{
