...
void configure_in_22ReadGrp2Var3TwoValues(DatabaseConfig*);
void configure_in_22ReadGrp2Var3TwoValues(DatabaseConfig* db)
{
}

void update_in_22ReadGrp2Var3TwoValues(IUpdateHandler*);
void update_in_22ReadGrp2Var3TwoValues(IUpdateHandler* db)
{
    std::cout << "update_in_22ReadGrp2Var3TwoValues" <<'\n';
...
////    };
 ...
 std::cout << "*Flags_In_FlagsOver2(&fFlags, 1)" <<'\n';
 std::cout << "*fFlags.value= "<<(uint16_t)fFlags.value<<'\n';

 DNPTime dDNPTime1;
 DNPTime_in_DNPTimeOver3(&dDNPTime1, 0x4571, TimestampQuality_SYNCHRONIZED);
 DNPTime dDNPTime2;
 DNPTime_in_DNPTimeOver3(&dDNPTime2, 0x4579, TimestampQuality_SYNCHRONIZED);

 Binary bBinary1;
 Binary_in_BinaryOver6(&bBinary1, false, fFlags, dDNPTime1);
 std::cout << "*Binary_in_BinaryOver6(&bBinary1, false, fFlags, dDNPTime1)" <<'\n';

 inspect_Binary(&bBinary1);

 Binary bBinary2;
 Binary_in_BinaryOver6(&bBinary2, true, fFlags, dDNPTime2);
 std::cout << "*Binary_in_BinaryOver6(&bBinary2, true, fFlags, dDNPTime2)" <<'\n';

 inspect_Binary(&bBinary2);
...
qDebug()<<"********SUITE('22ReadGrp2Var3TwoValues')********";
