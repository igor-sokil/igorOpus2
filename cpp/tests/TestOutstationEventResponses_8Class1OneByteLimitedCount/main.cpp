...
void configure_in_8Class1OneByteLimitedCount(DatabaseConfig*);
void configure_in_8Class1OneByteLimitedCount(DatabaseConfig* v)
{
 UNUSED(v);
}

void update_in_8Class1OneByteLimitedCount(IUpdateHandler*);
void update_in_8Class1OneByteLimitedCount(IUpdateHandler* db)
{
    std::cout << "update_in_8Class1OneByteLimitedCount1" <<'\n';
...
qDebug()<<"********SUITE('8Class1OneByteLimitedCount')********";
...
/*
 std::string request("C0 01 3C 02 07 01");       
 std::string response("E0 81 82 00 02 01 28 01 00 02 00 01");       
 TestEventRead(request, response,
                   ////const std::function<void(IUpdateHandler& db)>& loadFun,
                   update_in_8Class1OneByteLimitedCount,
                   ////const std::function<void(DatabaseConfig& db)>& configure = [](DatabaseConfig& view) {})
                   configure_in_8Class1OneByteLimitedCount);
*/
...
//    DatabaseConfig tmp = binary_input_in_DatabaseHelpers(1);
  DatabaseConfig database = all_types_in_DatabaseHelpers(3);
...
////    t.Transaction([&](IUpdateHandler& db) { loadFun(db); });
    Transaction_in_OutstationTestObject(&t, update_in_8Class1OneByteLimitedCount);//void (*apply)(IUpdateHandler*));

////    t.SendToOutstation(request);
    uint8_t name1[] = {6, 0xC0, 0x01, 0x3C, 0x02, 0x07, 0x01};
    SendToOutstation_in_OutstationTestObject(&t, name1);  
////    REQUIRE(t.lower->PopWriteAsHex() == response);
qDebug()<<"REQUIRE('E0 81 82 00 02 01 28 01 00 02 00 01' == t.lower->PopWriteAsHex())";

/*
TEST_CASE(SUITE("8Class1OneByteLimitedCount"))
{
    auto update = [](IUpdateHandler& db) {
        db.Update(Binary(false, Flags(0x01)), 2);
        db.Update(Binary(true, Flags(0x01)), 1);
