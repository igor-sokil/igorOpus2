...
qDebug()<<"********SUITE('18UnconfirmedUserData')********";
...
////    auto frame = LinkFrame::FormatUnconfirmedUserData(writeTo, true, 1, 2, data.ToRSeq(), nullptr);
RSeq_for_Uint16_t frame = FormatUnconfirmedUserData_in_LinkFrame_static(
  &writeTo, true, 1, 2, temp);//, Logger* pLogger);
...
////    REQUIRE(t.sink.CheckLast(LinkFunction::PRI_UNCONFIRMED_USER_DATA, true, 1, 2));
boolean tmp = CheckLast_in_MockFrameSink(&(t.sink),
    LinkFunction_PRI_UNCONFIRMED_USER_DATA, true, 1, 2);
qDebug()<<"REQUIRE(t.sink.CheckLast(LinkFunction::PRI_UNCONFIRMED_USER_DATA, true, 1, 2))";
qDebug()<<"tmp= "<<tmp;
