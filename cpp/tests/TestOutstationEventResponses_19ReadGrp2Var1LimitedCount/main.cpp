...
void configure_in_19ReadGrp2Var1LimitedCount(DatabaseConfig*);
void configure_in_19ReadGrp2Var1LimitedCount(DatabaseConfig* db)
{
}

void update_in_19ReadGrp2Var1LimitedCount(IUpdateHandler*);
void update_in_19ReadGrp2Var1LimitedCount(IUpdateHandler* db)
{
    std::cout << "update_in_19ReadGrp2Var1LimitedCount" <<'\n';
...
qDebug()<<"********SUITE('19ReadGrp2Var1LimitedCount')********";
...
////                  update); // 1 byte count == 1, ONLINE quality
 ...
 std::string response("E0 81 82 00 02 01 28 02 00 03 00 01 02 00 81");       

 std::string request("C0 01 02 01 07 02");       
 TestEventRead(request, response,
                   ////const std::function<void(IUpdateHandler& db)>& loadFun,
                   update_in_19ReadGrp2Var1LimitedCount,
                   ////const std::function<void(DatabaseConfig& db)>& configure = [](DatabaseConfig& view) {})
                   configure_in_19ReadGrp2Var1LimitedCount);
