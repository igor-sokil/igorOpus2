...
//  WSeq_for_Uint16_t* buffer, boolean aIsMaster, boolean aIsRcvBuffFull, uint16_t aDest, uint16_t aSrc);//, Logger* pLogger);
////    auto frame = LinkFrame::FormatNotSupported(writeTo, true, false, 1, 2, nullptr);
RSeq_for_Uint16_t frame = FormatNotSupported_in_LinkFrame_static(
...
boolean tmp = CheckLastWithDFC_in_MockFrameSink(&(t.sink),
    LinkFunction_SEC_NOT_SUPPORTED, true, false, 1, 2);
qDebug()<<"REQUIRE(t.sink.CheckLastWithDFC(LinkFunction::SEC_NOT_SUPPORTED, true, false, 1, 2))";
qDebug()<<"tmp= "<<tmp;
