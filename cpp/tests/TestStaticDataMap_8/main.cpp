...
qDebug()<<"********SUITE('8can select all points using specified variation and iterate')********";
...
uint32_t ctmp = select_all_in_StaticDataMap_for_BinarySpecOver2(&map, other_variation);
std::cout << "REQUIRE(map.select_all(other_variation) == 3)"<<'\n';
std::cout << "ctmp= " << ctmp<<'\n';
...
//    {
//        std::cout << "item.first= " << (uint16_t)item.first<<'\n';
...
std::cout << "REQUIRE(item.second.variation == other_variation)"<<'\n';
