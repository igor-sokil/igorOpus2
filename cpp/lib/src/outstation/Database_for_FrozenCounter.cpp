...
#ifdef  LOG_INFO
      std::cout<<getString_stack_info();
      std::cout<<"load_type_for_FrozenCounter_in_Database_static1_"<<'\n';
      decrement_stack_info();
#endif
      // there is no data left to write
      return true;
    }
    StaticFrozenCounterVariation_uint8_t variation = map->map[tmp].selection_in_StaticDataCell.variation;
    index++;
