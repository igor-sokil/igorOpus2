...
#include "StaticWriters_for_TimeAndInterval.h"
...
//----------------------------------------LoadWithRangeIterator_TimeAndIntervalSpec_for_UInt8---------------------------------------------------------
...
////    auto next_index = map.get_selected_range().start;
  uint16_t  next_index = get_selected_range_in_StaticDataMap_for_TimeAndIntervalSpec(map).start;

//  for (const auto& elem : map)
  int i = KeyMap2IndexMass_for_TimeAndIntervalSpec(map->db_config, next_index);
  while(i < MapSize_for_StaticDataMap_for_TimeAndIntervalSpec(map))
  {
    SelectedValue_for_TimeAndIntervalSpec elem = map->map[i].selection_in_StaticDataCell;
...
//    if (elem.first != next_index)
    if (i != KeyMap2IndexMass_for_TimeAndIntervalSpec(map->db_config, next_index))
    {
...
//    if (!Write_in_RangeWriteIterator_for_UInt8_TimeAndInterval(writer, &elem.second.value))
    if (!Write_in_RangeWriteIterator_for_UInt8_TimeAndInterval(writer, &elem.value))
    {
      return false;
    }

    ++next_index;
    i = iterator_operatorPlusPlus_for_TimeAndIntervalSpec(map, i);
  }

  return true;
}
//----------------------------------------LoadWithRangeIterator_TimeAndIntervalSpec_for_UInt8---------------------------------------------------------
//----------------------------------------LoadWithRangeIterator_TimeAndIntervalSpec_for_UInt16---------------------------------------------------------
////template<class Spec, class IndexType>
////bool LoadWithRangeIterator(StaticDataMap<Spec>& map,
////                           RangeWriteIterator<IndexType, typename Spec::meas_t>& writer,
////                           typename Spec::static_variation_t variation)
...
//    TimeAndInterval* value);
////        if (!writer.Write(elem.second.value))
//    if (!Write_in_RangeWriteIterator_for_UInt16_TimeAndInterval(writer, &elem.second.value))
    if (!Write_in_RangeWriteIterator_for_UInt16_TimeAndInterval(writer, &elem.value))
    {
      return false;
    }
...
{
////    const auto range = map.get_selected_range();
  Range  range = get_selected_range_in_StaticDataMap_for_TimeAndIntervalSpec(map);
...
////            QualifierCode::UINT8_START_STOP, Serializer::Inst(), static_cast<uint8_t>(range.start));
    ...
    RangeWriteIterator_for_UInt8_TimeAndInterval iter = IterateOverRange_for_UInt8_TimeAndInterval_in_HeaderWriter(writer,
...
//    RangeWriteIterator_for_UInt8_TimeAndInterval *writer, StaticTimeAndIntervalVariation_uint8_t variation)
////        return LoadWithRangeIterator<Spec, ser4cpp::UInt8>(map, iter, Serializer::svariation);
    boolean btmp = LoadWithRangeIterator_TimeAndIntervalSpec_for_UInt8_in_StaticWriters(map, &iter, StaticTimeAndIntervalVariation_Group50Var4);
    RangeWriteIterator_for_UInt8_TimeAndInterval_destr_RangeWriteIterator_for_UInt8_TimeAndInterval(&iter);
    return btmp;
  }
...
////                                                                                      Serializer::Inst(), range.start);
  DNP3Serializer_for_TimeAndInterval tmp = Inst_in_Group50Var4_static();
  RangeWriteIterator_for_UInt16_TimeAndInterval iter = IterateOverRange_for_UInt16_TimeAndInterval_in_HeaderWriter(writer,
      QualifierCode_UINT16_START_STOP,
      &tmp,
      (uint16_t) range.start );

////    return LoadWithRangeIterator<Spec, ser4cpp::UInt16>(map, iter, Serializer::svariation);
  boolean btmp = LoadWithRangeIterator_TimeAndIntervalSpec_for_UInt16_in_StaticWriters(map, &iter, StaticTimeAndIntervalVariation_Group50Var4);
  RangeWriteIterator_for_UInt16_TimeAndInterval_destr_RangeWriteIterator_for_UInt16_TimeAndInterval(&iter);
  return btmp;
}
//----------------------------------------------Group50Var4------------------------------------------
...
static_write_func_t_for_TimeAndIntervalSpec get_for_TimeAndIntervalSpec_in_StaticWriters_static(StaticTimeAndIntervalVariation_uint8_t variation)
{
  ...
  std::cout<<"*"<<getString_stack_info();
  std::cout<<"*WriteWithSerializer_TimeAndIntervalSpec_for_Group50Var4_in_StaticWriters_static"<<'\n';
  decrement_stack_info();
#endif
////        return &WriteWithSerializer<TimeAndIntervalSpec, Group50Var4>;
  return WriteWithSerializer_TimeAndIntervalSpec_for_Group50Var4_in_StaticWriters_static;
}
