...
////            if (!this->iterator.IsValid())
  ...
  {
    if ((meas->tTypedMeasurement_for_DoubleBit_uint8_t).mMeasurement.timeDNPTime_in_Measurement.quality != TimestampQuality_SYNCHRONIZED)
      return false;
  }
  else
  {
...
////            if (meas.time.value < this->cto.value)
  if ((meas->tTypedMeasurement_for_DoubleBit_uint8_t).mMeasurement.timeDNPTime_in_Measurement.value < parent->cto.value)
    return false;

////            const auto diff = meas.time.value - this->cto.value;
  uint64_t diff = (meas->tTypedMeasurement_for_DoubleBit_uint8_t).mMeasurement.timeDNPTime_in_Measurement.value - parent->cto.value;
...
////            copy.time = DNPTime(diff);
  copy.tTypedMeasurement_for_DoubleBit_uint8_t.mMeasurement.timeDNPTime_in_Measurement = dDNPTime;
