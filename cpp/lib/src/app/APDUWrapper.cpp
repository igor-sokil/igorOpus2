...
void APDUWrapper_in_APDUWrapperOver2(APDUWrapper *pAPDUWrapper, WSeq_for_Uint16_t *buffer)
{
  ...
  pAPDUWrapper->buffer_in_APDUWrapper = *buffer;
...
void SetFunction_in_APDUWrapper(APDUWrapper *pAPDUWrapper, FunctionCode_uint8_t code)
{
  ...
  std::cout<<"@@@@"<<getString_stack_info();
  std::cout<<"*write_uint8="<<(uint32_t)((pAPDUWrapper->buffer_in_APDUWrapper).buffer_+1)<<"->"<<(uint16_t)code<<'\n';
  std::cout<<getString_stack_info();
...
//    pAPDUWrapper->buffer[1] = code;////FunctionCodeSpec::to_type(code);
  (pAPDUWrapper->buffer_in_APDUWrapper).buffer_[1] = code;////FunctionCodeSpec::to_type(code);
}

FunctionCode_uint8_t GetFunction_in_APDUWrapper(APDUWrapper *pAPDUWrapper)
{
//    assert(buffer.is_not_empty());
  return from_type_in_FunctionCodeSpec((pAPDUWrapper->buffer_in_APDUWrapper).buffer_[1]);////(buffer[1]);
}
...
////    return AppControlField(buffer[0]);
  ...
  AppControlField_in_AppControlFieldOver2(&aAppControlField, (pAPDUWrapper->buffer_in_APDUWrapper).buffer_[0]);
#ifdef  LOG_INFO
  increment_stack_info();
  std::cout<<'\n';
  std::cout<<getString_stack_info();
  std::cout<<"{GetControl_in_APDUWrapper1"<<'\n';
  inspect_AppControlField(&aAppControlField);
  std::cout<<getString_stack_info();
  std::cout<<"}GetControl_in_APDUWrapper_"<<'\n';
  decrement_stack_info();
#endif
...
////    buffer[0] = control.ToByte();
  (pAPDUWrapper->buffer_in_APDUWrapper).buffer_[0] = ToByte_in_AppControlField(&control);
#ifdef  LOG_INFO
  increment_stack_info();
  std::cout<<'\n';
  std::cout<<getString_stack_info();
  ...
  std::cout<<"@@@@"<<getString_stack_info();
  std::cout<<"*write_uint8="<<(uint32_t)((pAPDUWrapper->buffer_in_APDUWrapper).buffer_+0)<<"->"<<(uint16_t)(pAPDUWrapper->buffer_in_APDUWrapper).buffer_[0]<<'\n';
  std::cout<<getString_stack_info();
...
////    return buffer.length() - remaining.length();
  uint16_t temp1 = length_in_HasLength_for_Uint16_t(&(pAPDUWrapper->buffer_in_APDUWrapper).hHasLength);
...
////    return buffer.readonly().take(this->Size());
  RSeq_for_Uint16_t temp1 = readonly_in_WSeq_for_Uint16_t(&(pAPDUWrapper->buffer_in_APDUWrapper));
